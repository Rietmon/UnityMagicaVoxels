...
{
	[Serializable]
    public class MagicaVoxelPalette
    {
        ...
        [field: SerializeField] private uint[] Colors { get; set; }
        
        public MagicaVoxelPalette(uint[] colors)
        {
	        Colors = colors ?? defaultPalette;
        }

        public Color32 GetColor(byte colorIndex) => ToColor(Colors[colorIndex - 1]);
        
        private static Color32 ToColor(uint hex) => new(
	        (byte)(hex >> 24 & 0xFF),
	        (byte)(hex >> 16 & 0xFF),
	        (byte)(hex >> 8 & 0xFF),
	        (byte)(hex & 0xFF));
